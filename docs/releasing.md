...
- Pushing the exact raw `<package.json version>` tag runs the same tests and build, uploads the workflow artifact, and creates a GitHub Release with the installer, OBS plugin, mod ZIPs, checksums, and the matching reviewed `docs/releases/v<tag>.md` public note. Every public note must have a matching `docs/changelogs/v<tag>.md` technical changelog and an entry in `docs/releases/index.md`; the release-documentation validator checks the pairing, required sections, concise-note size, and GitHub Release-safe links before publication. Git refs use raw SemVer such as `0.3.1`; player-facing build labels, documentation filenames, workflow artifacts, and Release titles retain `v0.3.1`.
- Tags containing `-` (for example, `0.4.0-beta.1`) publish as prereleases;
  stable tags such as `0.3.1` publish as official releases.

Historical `v`-prefixed Git refs were migrated to raw SemVer. Do not recreate
the aliases: two names for one release make update provenance and operator
instructions ambiguous. The updater accepts one historical prefix only for
backward compatibility with cached release metadata.
...
```powershell
git tag -a 0.3.1 -m "Beatblock Online v0.3.1"
