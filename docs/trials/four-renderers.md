...
From the in-game roster and **Broadcast** workspace, assign four different Players to Streams A-D. Run the host as a Player while all slots render 1280x720 at 60 fps with a 500 ms buffer for ten minutes. Repeat from a host-granted Commentator mirror. Exercise blocks, taps, holds, mines, multi-paddle changes, Full and Clean mode, featured switching, and stream reassignment. Confirm stable slot names and separate Host Plan/This PC health. Pass with under 1% renderer frame drops and no material host-player frame-time regression. Repeat mixed FPS and 1080p configurations and verify warnings plus graceful degradation.
